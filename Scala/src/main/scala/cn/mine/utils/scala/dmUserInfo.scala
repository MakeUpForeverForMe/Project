...
import cn.mine.utils.scala.UtilsGet._
...
object dmUserInfo {
    ...
    // 生成数据的数量
    private val amount = 5 * 10 * 10 * 10 * 10 * 10
    // 性别
    ...
    def produce(): ListBuffer[String] = {
        ...
        for (amount <- 0 until amount) {
            ...
            val idcard_aes = AesUtils.encrypt(f"$idcard%18s", AesUtils.PASSWORD_TENCENT)
            val moblie_aes = AesUtils.encrypt(f"$mobile%11s", AesUtils.PASSWORD_TENCENT)
            val name_aes = AesUtils.encrypt(f"$mobile%11s", AesUtils.PASSWORD_TENCENT)
            val birth = DateUtils.DATE_TIME_LINE.transform(births, DateUtils.DATE_TIME_STAMP.formatter)
