...
object ConsumerSystemActors {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val actorRef1 = actorSystem.actorOf(Props(new ConsumerActor(clusterName, MuHashMap((actorName2, hp)))), actorName1)
    val actorRef2 = actorSystem.actorOf(Props(new ConsumerActor(clusterName, MuHashMap((actorName1, hp)))), actorName2)
