...
import cn.mine.ImuMap
import cn.mine.actor.ConsumerActor
...
object ConsumerSystemCluster {
    ...
    private val clusterName: String = properties.getProperty("cluster.name")
    private val nodeName: String = properties.getProperty("whoami")

    // node1@127.0.0.1:9991  nhpMap --> (node, (host, port))
    private val nodeHPMap: ImuMap = properties.getProperty("cluster.nodes").split(",").map {
        nhp =>
