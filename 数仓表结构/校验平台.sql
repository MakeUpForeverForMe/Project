...
`t_loan_contract_info`      `asset_01_t_loan_contract_info`      '文件01' '贷款合同信息表'
`t_principal_borrower_info` `asset_02_t_principal_borrower_info` '文件02' '主借款人信息表'

`t_contact_person_info`     `asset_03_t_contact_person_info`     '文件03' '主借款人联系人信息表'
`t_guaranty_info`           `asset_04_t_guaranty_info`           '文件04' '抵押物信息表' -- 文件13也在这里，用类型区分  -- 有Excel导入
`t_enterprise_info`         `asset_12_t_enterprise_info`         '文件12' '企业名称信息表-文件十二'

`t_repayment_schedule`      `asset_05_t_repayment_schedule`      '文件05' '还款计划信息表'  -- 有Excel导入
`t_asset_pay_flow`          `asset_06_t_asset_pay_flow`          '文件06' '资产支付流水信息表'
`t_repayment_info`          `asset_07_t_repayment_info`          '文件07' '实际还款信息表'  -- 有Excel导入
`t_asset_check`             `asset_10_t_asset_check`             '文件10' '资产对账信息表'  -- 有Excel导入

`t_asset_disposal`          `asset_08_t_asset_disposal`          '文件08' '资产处置过程信息表'         -- 不需要
`t_asset_supplement`        `asset_09_t_asset_supplement`        '文件09' '资产补充交易信息表'         -- 不需要
`t_project_check`           `asset_11_t_project_check`           '文件11' '项目对账信息表-文件十一'    -- 不需要
