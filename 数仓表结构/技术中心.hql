...
where 1 > 0
  and case when length(map_from_str(extra_info)['合同结束时间']) = 19 then to_date(map_from_str(extra_info)['合同结束时间'])
  else is_empty(datefmt(map_from_str(extra_info)['合同结束时间'],'','yyyy-MM-dd')) end
  <
  is_empty(
    case when length(map_from_str(extra_info)['最后一次预计扣款时间']) = 19 then to_date(map_from_str(extra_info)['最后一次预计扣款时间'])
...
select
  is_empty(map_from_str(extra_info)['担保方式']) as a1,
...
select distinct
  ...
  concat('--=',is_empty(map_from_str(extra_info)['担保方式'],guarantee_type),'=--') as guarantee_type
from stage.asset_01_t_loan_contract_info
limit 10;
...
select is_empty('null','na','中文') as aa;
...
ALTER TABLE dim.dim_investor_info SET TBLPROPERTIES('EXTERNAL'='false');
...
select max(biz_date) from dm_eagle.abs_asset_distribution_bag_day;

select max(e_d_date) from ods.loan_info_abs;


select distinct project_id from ods.t_10_basic_asset;
...
where 1 > 0
  and biz_date = '${ST9}'
  and size(map_keys(dpd_map)) = 0
  -- and overdue_days = 0
  and project_id = 'pl00282'
...
select
  ...
  t1.effective_date,
  t1.biz_date,
  if(t1.loan_init_term > t2.after_init_term,t2.after_biz_date,null) as e_d_date,
  t1.product_id
from (
  select
    due_bill_no,
...
where 1 > 0
  and product_id = 'CL202101260095'
  and due_bill_no in ('GALC-HL-1609070134','GALC-HL-1612060464')
order by product_id,due_bill_no,loan_term,s_d_date
;
...
select
  ...
  due_bill_no,
  loan_init_term,
  count(distinct loan_init_term) as term_cnt,
  count(due_bill_no) as cnt
from ods.repay_schedule_inter
group by product_id,due_bill_no,loan_init_term
having count(distinct loan_init_term) > 1
limit 10;
...
from (
  select
    ...
    product_id
  from (
    select '123' as product_id,'abc' as due_bill_no,'60' as loan_init_term,'2021-01-01' as biz_date union all
    select '123' as product_id,'abc' as due_bill_no,'60' as loan_init_term,'2021-02-01' as biz_date union all
    select '123' as product_id,'abc' as due_bill_no,'54' as loan_init_term,'2021-03-01' as biz_date union all
    select '123' as product_id,'abc' as due_bill_no,'54' as loan_init_term,'2021-04-01' as biz_date union all
    select '123' as product_id,'abc' as due_bill_no,'48' as loan_init_term,'2021-05-01' as biz_date union all
    select '123' as product_id,'abc' as due_bill_no,'48' as loan_init_term,'2021-06-01' as biz_date
  ) as tmp
) as tmp
order by biz_date
;
