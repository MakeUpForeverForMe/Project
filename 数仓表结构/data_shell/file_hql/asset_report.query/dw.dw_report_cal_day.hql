...
--累计静态逾期金额
insert overwrite table dw.dw_report_cal_day partition (biz_date='${ST9}')
select
--case
...
(
    select distinct
       ...
       from (
         ...
         where col_type = 'ac'
         group by col_id
    )tmp where product_id in (${product_id_list})
)biz_conf
left join
(
select
...
from dim.dim_static_overdue_bill where prj_type in ('LXGM','LXZT','LXGM2') and biz_date<= '${ST9}' and cast(overdue_days as int) in (31,91)
group by  product_id
...
from
(
select * from   dim.dim_static_overdue_bill where biz_date<='${ST9}' and prj_type in ('LXGM','LXZT','LXGM2')  and cast(overdue_days as int) in (31,91)
...
left join(
    ...
    from ods_cps.repay_schedule where
