...
create table if not exists `dm_eagle.eagle_ret_msg_day`(
  ...
  `ret_msg_rate`                                  decimal(9,0)   COMMENT '拒绝原因占比（各个拒绝原因笔数/总的拒绝笔数）'
) COMMENT '拒绝原因分布'
...
create table if not exists `dm_eagle.assets_distribution`(
  ...
  `bill_credit_line`                              decimal(15,0)  COMMENT 'bill-贷款额度',
  `bill_term`                                     decimal(5,0)   COMMENT 'bill-贷款总期数',
  `bill_interest_rate`                            decimal(15,8)  COMMENT 'bill-利率',
...
create table if not exists `dm_eagle${db_suffix}.eagle_inflow_rate_day`(
  ...
  `overdue_loan_num`                              decimal(11,0)  COMMENT '逾期借据数',
  `overdue_loan_inflow_rate`                      decimal(15,8)  COMMENT '案件逾期率（逾期借据数/应还日为T，T-1为C的T-1日应还借据数×100%）',
  `loan_active_month`                             string         COMMENT '放款月份',
  `should_repay_date`                             string         COMMENT '应还日期',
  `remain_principal`                              decimal(15,4)  COMMENT '应还日本金余额（应还日为T，T-1为C的T-1日实际应还本金）',
  `should_repay_principal`                        decimal(15,4)  COMMENT '应还日应还本金（应还日为T，T-1为C的T-1日实际本金余额）',
  `overdue_principal`                             decimal(15,4)  COMMENT '逾期借据逾期本金',
  `overdue_remain_principal`                      decimal(15,4)  COMMENT '逾期借据本金余额',
  `overdue_principal_inflow_rate`                 decimal(15,8)  COMMENT '逾期借据逾期本金逾期率（逾期借据逾期本金/应还日为T，T-1为C的T-1日实际应还本金×100%）',
  `remain_principal_inflow_rate`                  decimal(15,8)  COMMENT '逾期借据本金余额逾期率（逾期借据本金余额/应还日为T，T-1为C的T-1日实际本金余额×100%）'
...
CREATE TABLE IF NOT EXISTS `dm_eagle${db_suffix}.eagle_loan_info`(
  ...
  `loan_penalty_accumulate`                       decimal(15,4)  COMMENT '累计罚息',
...
CREATE TABLE IF NOT EXISTS `dm_eagle${db_suffix}.eagle_repay_schedule`(
  ...
  `repaid_num`                                    decimal(15,0)  COMMENT '实还笔数',
  `paid_amount`                                   decimal(15,4)  COMMENT '已还金额',
  `paid_principal`                                decimal(15,4)  COMMENT '已还本金',
  `paid_interest`                                 decimal(15,4)  COMMENT '已还利息',
  `paid_term_fee`                                 decimal(15,4)  COMMENT '已还手续费',
...
CREATE TABLE IF NOT EXISTS `dm_eagle${db_suffix}.eagle_repayment_record_day`(
  ...
  `paid_amount`                                   decimal(15,4)  COMMENT '实还金额',
  `paid_principal`                                decimal(15,4)  COMMENT '实还本金',
  `paid_interest`                                 decimal(15,4)  COMMENT '实还利息',
  `paid_term_svc_fee`                             decimal(15,4)  COMMENT '实还费用',
  `paid_penalty`                                  decimal(15,4)  COMMENT '实还罚息',
  `remain_principal`                              decimal(15,4)  COMMENT '本金余额',
...
create table if not exists `dm_eagle.operation_overdue_base_datail`(
  ...
  `remain_fee`                                    decimal(15,4)  COMMENT '剩余未还费用',
  `remain_penalty_interest`                       decimal(15,4)  COMMENT '剩余未还罚息',
  `remain_amount_cps`                             decimal(15,4)  COMMENT '剩余未还金额(代偿后)',
  `remain_principal_cps`                          decimal(15,4)  COMMENT '剩余未还本金(代偿后)',
  `remain_interest_cps`                           decimal(15,4)  COMMENT '剩余未还利息(代偿后)',
  `remain_fee_cps`                                decimal(15,4)  COMMENT '剩余未还费用(代偿后)',
  `remain_penalty_interest_cps`                   decimal(15,4)  COMMENT '剩余未还罚息(代偿后)',
...
create table if not exists `dm_eagle.operation_lending_daily_agg` (
  ...
  `loan_num`                                      decimal(15,4)  COMMENT '放款日放款笔数',
  `loan_principal`                                decimal(15,4)  COMMENT '放款日放款金额',
  `loan_num_count`                                decimal(10,0)  COMMENT '累计放款笔数',
  `loan_principal_count`                          decimal(15,4)  COMMENT '累计放款金额',
  `execution_date`                                string         COMMENT '跑批日期'
) COMMENT '放款日报'
...
CREATE TABLE IF NOT EXISTS `dm_eagle.abs_overdue_rate_details_day`(
  ...
  `loan_terms`                                    decimal(3,0)   COMMENT '贷款期数',
  `loan_principal`                                decimal(20,5)  COMMENT '合同金额',
  `remain_term`                                   decimal(3,0)   COMMENT '剩余期数',
  `overdue_days_max`                              decimal(4,0)   COMMENT '最长单期逾期天数',
  `remain_principal`                              decimal(20,5)  COMMENT '当前本金余额（元）',
  `overdue_principal`                             decimal(20,5)  COMMENT '当前逾期本金（元）',
  `overdue_interest`                              decimal(20,5)  COMMENT '当前逾期利息（元）',
  `overdue_cost`                                  decimal(20,5)  COMMENT '当前逾期费用（元）'
) COMMENT '逾期情况详情（借据详情）'
...
CREATE TABLE IF NOT EXISTS `dm_eagle.abs_early_payment_asset_details`(
  ...
  `serial_number`                                 string         COMMENT '借据号',
  `contract_no`                                   string         COMMENT '合同编号',
  `remain_principal_before_payment`               decimal(15,4)  COMMENT '还款前剩余本金',
  `early_payment_date`                            string         COMMENT '提前还款日期',
  `early_payment_amount`                          decimal(15,4)  COMMENT '总金额',
  `early_payment_principal`                       decimal(15,4)  COMMENT '提前结清实还本金',
  `early_payment_interest`                        decimal(15,4)  COMMENT '提前结清实还利息',
  `early_payment_fee`                             decimal(15,4)  COMMENT '提前结清实还费用'
) COMMENT '早偿监控资产详情表'
PARTITIONED BY(`biz_date` string COMMENT '快照日',`project_id` string COMMENT '项目编号',`bag_id` string COMMENT '包编号')
STORED AS PARQUET;
