...
select 
  ...
  a.interest_rate        as loan_init_interest_rate,
  ...
  a.init_contract_amount as loan_init_amount,
...
(select
*,
row_number() over(partition by due_bill_no order by batch_date desc) rn from stage.loan_contract_ex where d_date<='${ST9}' and p_type='WS0013200001') a
where a.rn=1
 ) b
