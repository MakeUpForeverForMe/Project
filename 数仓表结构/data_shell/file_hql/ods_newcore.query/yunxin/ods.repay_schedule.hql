...
where 1 > 0
  and is_settled = 'no'
  and product_id in ('002601','002602')
  and s_d_date < '${ST9}';
...
b.term_int                                                                      as should_repay_interest,
...
b.term_repay_int                                                               as paid_interest,
0                                                                              as paid_term_fee,
0                                                                              as paid_svc_fee,
...
b.term_reduce_int                                                               as reduce_interest,
...
(select
 *
from stage.${tb_prefix}repayment_plan where d_date='${ST9}' and p_type='WS0013200001') b
...
(select
  ...
  term,
  product_no
from stage.${tb_prefix}receipt_detail where d_date<='${ST9}' and p_type='WS0013200001' and term !=999 and receipt_type !='REDUCE'
...
a.product_id
from
(select * from ods${db_suffix}.repay_schedule_tmp where product_id in ('002601','002602')) a
left join
...
left join
(select * from stage.loan_contract where d_date='${ST9}' and p_type='WS0013200001') c
