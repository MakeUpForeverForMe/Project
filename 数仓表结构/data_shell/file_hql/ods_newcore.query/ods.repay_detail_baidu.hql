...
''                                   as post_time,
case a.fee_type
  when 'SVCFee'      then 'Interest'
  else a.fee_type    end             as bnp_type,
case a.fee_type
...
left join
(select * from stage.${tb_prefix}repayment_plan where d_date='${ST9}' and p_type='WS0012200001') c
on a.due_bill_no=c.due_bill_no and a.product_no=c.product_no and a.term=c.term
left join
(select * from stage.${tb_prefix}repayment_summary where d_date='${ST9}' and p_type='WS0012200001') d
on a.due_bill_no=d.due_bill_no and a.product_no=d.product_no
left join
(select * from stage.${tb_prefix}overdue_summary where d_date='${ST9}' and p_type='WS0012200001') e
