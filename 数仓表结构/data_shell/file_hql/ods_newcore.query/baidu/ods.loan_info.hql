...
nvl(f.paid_fee,0)                                                      as paid_interest,
...
if(b.asset_status='SETTLED',0,f.should_fee-nvl(f.paid_fee,0)-nvl(f.reduce_fee,0))                      as remain_interest,
0                                                                          as remain_svc_fee,
...
d.current_overdue_fee                                                      as overdue_interest,
...
datefmt(b.created_date, 'ms','yyyy-MM-dd HH:mm:ss')                        as create_time,
datefmt(b.last_modified_date, 'ms','yyyy-MM-dd HH:mm:ss')                  as update_time,
...
(select
 *
from stage.${tb_prefix}repayment_summary where d_date='${ST9}' and p_type='WS0012200001') b
...
from stage.${tb_prefix}overdue_summary where d_date<='${ST9}' and p_type='WS0012200001') a
where a.rn=1) d
...
from stage.${tb_prefix}repayment_plan where d_date<='${ST9}' and p_type='WS0012200001' ) a
where a.rn =1) a
left join
(
select
...
from stage.loan_contract where d_date<='${ST9}' and p_type='WS0012200001' ) a
where a.rn=1
) b
...
a.product_id
from
(select * from ods${db_suffix}.loan_info_tmp where product_id ='002501') a
left join
