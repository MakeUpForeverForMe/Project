...
-- 滴滴
insert into table ods.linkman_info partition (product_id)
select distinct
...
join (
  select
    ...
    case
      when relationalHuman['relationalHumanType'] = 'RHT01' then '借款人联系人'
    ...
    case
      when relationalHuman['relationship'] = 'C' then '配偶'
...
insert overwrite table ods.linkman_info partition(product_id)
select distinct * from ods.linkman_info
