...
from (
  select
    ...
    age_birth(customer_info.birthday,to_date(loan_result.issuetime)) as age,
    null                                             as pre_apply_no,
    loan_apply.credit_id                             as apply_id,
    loan_apply.loan_order_id                         as due_bill_no,
    cast(least(nvl(loan_result.issuetime,'9999-99-99'),nvl(loan_apply.create_time,'9999-99-99')) as timestamp) as loan_apply_time,
    ...
    cast(least(to_date(nvl(loan_result.issuetime,'9999-99-99')),loan_apply.deal_date) as string) as biz_date,
...
from (
  ...
  from (
    select
      ...
      age_birth(nms_apply.birthday,to_date(nms_loan.loan_date))          as age,
      ...
      case nms_loan.apply_status when 4 then cast(nms_loan.loan_amt as decimal(15,8))
      ...
      cast(nvl(nms_apply.deal_date,nms_loan.deal_date) as string)        as biz_date,
      nvl(nms_apply.product_no,nms_loan.product_no)                      as product_id
    from (
      select
        deal_date                                                                 as deal_date,
        datefmt(create_time,'ms','yyyy-MM-dd HH:mm:ss')                           as create_time,
...
from ods_new_s.loan_apply
where 1 > 0
  and biz_date in ${biz_date_lx}
  and product_id in ('001801','001802','001803','001804','001901','001902','001903','001904','001905','001906','001907','002001','002002','002003','002004','002005','002006','002007','002401','002402')
;
  ...
  from (
    select
      ...
      age_birth(
        ...
        to_date(nvl(ecas_loan.active_date,get_json_object(loan_apply.original_msg,'$.reqContent.jsonReq.content.reqData.loanDate')))
      )                                                                                                               as age,
      ...
      cast(nvl(get_json_object(loan_apply.original_msg,'$.reqContent.jsonReq.content.reqData.creditCoef'))as decimal(15,8))*12/100 as credit_coef,
      0                                                                                                               as penalty_rate,
      case
    ...
    left join (
      ...
      where 1 > 0
        ...
        --and d_date in (date_sub(current_date,1),date_sub(current_date,2))
    ) as ecas_loan
...
from ods_new_s.loan_apply
where 1 > 0
  and biz_date in ${biz_date_gz}
  and product_id in ('001701','001702')
;
...
from (
  select
    ...
    age_birth(datefmt(substring(get_json_object(loan_apply.original_msg,'$.data.borrower.idNo'),7,8),'yyyyMMdd','yyyy-MM-dd'),to_date(loan_result.issue_time))   as age,
    ...
    when 'LAU99' then '其他类消费'
    else get_json_object(credit_apply.original_msg,'$.data.product.loanApplyUse') end                                                           as loan_usage_cn,
    ...
    case loan_result.apply_status when 4 then cast(get_json_object(credit_apply.original_msg,'$.data.product.loanAmt') as decimal(15,4))
    else 0 end                                                                                                                                  as loan_amount_approval,
    ...
    loan_apply.update_time                                                                                                                      as update_time,
    cast(least(to_date(loan_apply.create_time),datefmt(get_json_object(loan_apply.original_msg,'$.timeStamp'),'ms','yyyy-MM-dd')) as string)    as biz_date,
  ...
  left join (
    select
      ...
      is_empty(
        ...
        when '5' then '用信失败'
        when '7' then '申请拒绝'
        else '未知原因' end)                                                   as apply_resut_msg,
