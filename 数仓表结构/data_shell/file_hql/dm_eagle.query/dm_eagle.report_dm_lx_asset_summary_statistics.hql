upsert into dm_eagle.report_dm_lx_asset_summary_statistics
select
cast(concat(from_unixtime(UNIX_TIMESTAMP('${var:ST9}'),'yyyyMMdd'),project_id) as String) as id,
project_id,
...
from  dw_new.dw_report_cal_day where biz_date=date_add('${var:ST9}',-1)
group by project_id
)temp;
