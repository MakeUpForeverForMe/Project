...
from
    dim.bag_due_bill_no  as bag
    inner join  bill_info on bag.due_bill_no =bill_info.due_bill_no
...
from
    dim.project_due_bill_no as project
...
group by asset_tab_name,asset_name,asset_name_order,baginfo.project_id
union all
select
'n'                                                                                                                                as is_allbag,
asset_tab_name                                                                                                                     as asset_tab_name,
...
sum(remain_principal)                                                                                                              as remain_principal,
sum(remain_principal)/max(project_total.total_remain_principal)                                                                    as remain_principal_ratio,
count(project.due_bill_no)                                                                                                         as loan_num,
count(project.due_bill_no)/max(project_total.total_bill)                                                                           as loan_numratio,
sum(remain_principal)/count(project.due_bill_no)                                                                                   as remain_principal_loan_num_avg,
'${ST9}'                                                                                                                       as biz_date,
...
)tmp on project.due_bill_no =tmp.due_bill_no
inner join project_total_bill as project_total on project.project_id=project_total.project_id
where asset_name!="NONE"
group by asset_tab_name,asset_name,asset_name_order,project.project_id
-- limit 10
;
