...
(
    ...
    select
        ...
        project_id
    from
        dim_new.bag_info where bag_date='${ST9}'
    )bag inner join
    dim_new.bag_due_bill_no bag_bill on bag.bag_id=bag_bill.bag_id
)bag_info
inner join
(
    select
...
)loan_info on bag_info.due_bill_no=loan_info.due_bill_no
left join
(
select
    due_bill_no ,
...
select
    ...
    count(due_bill_no)                as total_bill
from
     bill_info
...
count(due_bill_no)/max(project_total.total_bill)                                                                                   as loan_numratio,
sum(remain_principal)/count(due_bill_no)                                                                                           as remain_principal_loan_num_avg,
'${ST9}'                                                                                                                           as biz_date,
 tmp.bag_id                                                                                                                        as bag_id
from
(
    select
        due_bill_no,
...
inner join project_total_bag_bill as project_total on tmp.project_id=project_total.project_id
and tmp.bag_id=project_total.bag_id
where asset_name!="NONE"
group by asset_tab_name,asset_name,asset_name_order,tmp.project_id,tmp.bag_id
