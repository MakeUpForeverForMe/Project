...
with bill_info as (
  ...
  from (
    select
      ...
      loan_info_abs.loan_term_remain,
      loan_info_abs.loan_term_repaid,
      loan_info_abs.account_age,
      bag_info.project_id,
      bag_info.bag_id,
      bag_info.bag_date,
      bag_due.due_bill_no
    ...
    and bag_info.bag_date between loan_info_abs.s_d_date and date_sub(loan_info_abs.e_d_date,1)
...
from (
  ...
  group by project_id,bag_id,bag_date,split(part_info,'\\|')[0],split(part_info,'\\|')[1],split(part_info,'\\|')[2]
) as asset_total
inner join (
  select
