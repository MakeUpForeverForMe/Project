...
insert overwrite table dm_eagle.abs_asset_information_project partition(biz_date='${ST9}',project_id)
select
...
on pro_due.due_bill_no = loan.due_bill_no
...
select
    ...
    'n'                                                                                                                        as is_allBag,
    pro_due.project_id                                                                                                         as project_id
from dim.project_due_bill_no pro_due
inner join (
    select * from ods.loan_info
    where '${ST9}' between s_d_date and date_sub(e_d_date,1)
      and loan_status<>'F'
