--放款统计 日表
--create table if not exists eagle.eagle_loan_base_stat_loan_num_d(
--     project_id                           string             comment '项目ID'
--    ,loan_terms                           decimal(4,0)       comment '贷款期数'
--    ,loan_principal                       decimal(15,4)      comment '贷款金额' 
--) comment '放款统计-日表'
...
insert overwrite table eagle.eagle_loan_base_stat_loan_num_d partition(biz_date='${ST9}',product_id)
select
    t2.project_id
    ,t1.loan_terms
    ,t1.loan_principal
