--create table if not exists eagle.eagle_loan_apply_stat_w(
...
--    ,weig_interest_rate                   decimal(11,4)      comment '当周放款加权利率'
--
...
select
    product_id
    ,count( distinct (if(loan_apply_num>0,user_hash_no, null)))           as loan_apply_num_person
    ,count( distinct (if(loan_approval_num>0,user_hash_no,null)))         as loan_approval_num_person
