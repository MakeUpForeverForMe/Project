--放款统计 周表
--create table if not exists eagle.eagle_loan_base_stat_loan_num_w(
--     project_id                           string             comment '项目ID'
--    ,loan_terms                           decimal(4,0)       comment '贷款期数'
--    ,loan_principal                       decimal(15,4)      comment '当周贷款金额-从周一到周日' 
--) comment '放款统计-周表'
...
insert overwrite table eagle.eagle_loan_base_stat_loan_num_w partition(biz_date,product_id)
select
    t2.project_id
    ,t1.loan_terms
