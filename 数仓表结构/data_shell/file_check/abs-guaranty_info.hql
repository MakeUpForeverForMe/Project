...
from (
  ...
  where 1 > 0
    and project_id in (select distinct is_empty(map_from_str(extra_info)['项目编号'],project_id) from ods.t_guaranty_info)
    and if(project_id = 'CL202101260095',guaranty_code is null,true)
) as stage
left join (
  select distinct
  ...
  and is_empty(map_from_str(extra_info)['抵押物编号'],guaranty_umber) = t_04_guaranty_code
) as ods
