...
-- ods 放款 与 dw 放款 对比 借据数、放款金额 （每天对比全量）
select
  'ods 放款 与 dw 放款 对比 借据数、放款金额' as title,
  ...
  nvl(ods.loan_original_principal,0)                                     as principal_ods,
...
where 
nvl(ods.loan_num,0) - nvl(dw.loan_num,0)                      != 0 or 
