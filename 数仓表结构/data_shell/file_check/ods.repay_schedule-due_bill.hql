...
select
  'ods stage repay_schedule due'                                              as title,
  nvl(ods_inter.project_id,ods.project_id)                                    as project_id,
  nvl(ods_inter.loan_term,ods.loan_term)                                      as term,
  nvl(ods_inter.should_repay_date,ods.should_repay_date)                      as should_repay_date,
  nvl(ods_inter.due_bill_no,ods.due_bill_no)                                  as due,
  nvl(ods_inter.due_bill_no,null)                                             as due_ods_inter,
  nvl(ods.due_bill_no,null)                                                   as due_ods,
  nvl(ods_inter.should_repay_principal,ods.should_repay_principal)            as principal,
...
and ods_inter.due_bill_no       = ods.due_bill_no
...
where 1 > 0
  ...
  and nvl(ods_inter.project_id,ods.project_id) in (
    'cl00297',
    'cl00306',
    'cl00309',
    'CL201911130070',
  ...
  and (
    nvl(ods_inter.due_bill_no,null) is null or
    nvl(ods.due_bill_no,null)       is null or
    nvl(ods_inter.should_repay_principal,0) - nvl(ods.should_repay_principal,0) != 0 or
    false
  )
