...
echo -e "${date_s_aa:=$(date +'%F %T')} abs资产 dm_eagle解包  开始\n" &>> $log
# dm 层
yuheng=$wy

sh $data_manage -s ${s_date} -e ${e_date} -f $dm_eagle_hql/dm_eagle.abs_asset_information_unpacket.hql -i $param_dir/dm_eagle_abs.param.hql -k bag_id="${bag_id}" -b "AssetFileToHive-${bag_id}" -a $yuheng &
...
echo -e "${date_e_aa:=$(date +'%F %T')} abs资产 dm_eagle解包 结束    用时：${during_time:=$(during "$date_e_aa" "$date_s_aa")}\n\n" &>> $log
